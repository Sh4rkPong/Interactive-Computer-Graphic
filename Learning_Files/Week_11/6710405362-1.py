...
void main()
{
    ...
    vec3 diffuse = Kd * light_intensity * max(dot(N, L), 0.0);
    vec3 specular = vec3(0.0);
    if (dot(N, L) > 0.0) {
        specular = Ks * light_intensity * pow(max(dot(R, V), 0.0), shininess);
    }
...
void main()
{
    ...
    vec3 L = normalize(light_pos - fragWorldPos);
    vec3 V = normalize(eye_pos - fragWorldPos);
    vec3 R = reflect(-L, N);
