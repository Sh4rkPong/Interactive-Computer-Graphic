...
def draw_gui():
    ...
    global lighting_on, texture_on, texture_choice, specular_with_texture
    ...
    imgui.text("Lighting")
    _, lighting_on = imgui.checkbox("Lighting Enabled", lighting_on)
    ...
    _, specular_with_texture = imgui.checkbox("Specular over Texture", specular_with_texture)
    ...
    imgui.text("Texture")
    _, texture_on = imgui.checkbox("Texture Enabled", texture_on)
    texture_keys = list(textures.keys())
    current_index = texture_keys.index(texture_choice) if texture_choice in texture_keys else 0
    clicked, current_index = imgui.combo("Texture Map", current_index, texture_keys)
    texture_choice = texture_keys[current_index]
...
def refresh(window):
    ...
    glLightfv(GL_LIGHT0, GL_SPECULAR, I if specular_on else [0, 0, 0])

    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, (*Ka, 1))
    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, (*I, 1) if specular_on else (0, 0, 0, 1))
    glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, shininess)

    if texture_on:
        glEnable(GL_TEXTURE_2D)
        glBindTexture(GL_TEXTURE_2D, textures[texture_choice])
        glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE)
    else:
        glDisable(GL_TEXTURE_2D)

    if specular_with_texture:
...
def gl_init():
    ...
    global impl, eye_pos, eye_at, textures
    ...
    textures = {
        "bunny_hair": load_texture("../texture_map/bunny_hair.jpg"),
        "black": load_texture("../texture_map/black.jpg"),
    }
